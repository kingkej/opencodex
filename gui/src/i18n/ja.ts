...
export const ja: Record<TKey, string> = {
  ...
  "codexAuth.accountPickerOffDesc": "有効にすると、アカウントセレクターごとの項目がピッカーに追加され、ログアウトせずに会話で使うアカウントを明示的に選べます。通常の自動 Pool 項目を並べて表示するかどうかは、2 つ目の設定で選べます。アカウント指定を無効にしてもアカウントは削除されません。",
  ...
  "codexAuth.accountPickerPoolModelsTitle": "自動 Pool モデルをピッカーに残す",
  "codexAuth.accountPickerPoolModelsOffDesc": "アカウント指定が有効な間は、アカウントセレクターだけが表示されます。通常の自動 Pool 項目も利用できるようにするには、この設定を有効にします。",
  "codexAuth.accountPickerPoolModelsOnDesc": "通常の Pool モデル項目がアカウントセレクターと並んで表示されます。OpenCodex に自動でローテーションやフォールバックを任せる場合に選択します。",
  "codexAuth.accountPickerPoolModelsUpdated": "自動 Pool モデルの表示設定を更新しました。",
  "codexAuth.accountPickerPoolModelsUpdateFailed": "自動 Pool モデルの表示設定を更新できませんでした。最後に確認された設定を表示しています。",
