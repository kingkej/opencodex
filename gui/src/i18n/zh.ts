...
export const zh: Record<TKey, string> = {
  ...
  "codexAuth.accountPickerOffDesc": "启用后，模型选择器会为每个账号选择器添加一个条目，让你无需退出登录即可为对话明确选择账号。另一个设置决定是否同时保留普通的自动 Pool 条目。关闭账号指定功能不会删除任何账号。",
  ...
  "codexAuth.accountPickerPoolModelsTitle": "在选择器中保留自动 Pool 模型",
  "codexAuth.accountPickerPoolModelsOffDesc": "启用账号指定时，仅显示账号选择器。开启此设置可同时保留普通的自动 Pool 条目。",
  "codexAuth.accountPickerPoolModelsOnDesc": "普通 Pool 模型条目会与账号选择器同时显示。希望 OpenCodex 自动轮换或故障转移时，请选择这些条目。",
  "codexAuth.accountPickerPoolModelsUpdated": "自动 Pool 模型的显示设置已更新。",
  "codexAuth.accountPickerPoolModelsUpdateFailed": "无法更新自动 Pool 模型的显示设置。当前显示的是最后一次确认的设置。",
