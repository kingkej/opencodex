...
| `codexAccountPickerShowPoolModels?` | `boolean` | `false` | 계정 한정 행이 활성화되어 있을 때 자동 Pool/Direct native 행도 함께 표시합니다. bare 행은 일반 Pool 라우팅과 fallback을 사용하고, 한정 행은 지정 계정만 사용합니다. |
