...
| `codexAccountPickerShowPoolModels?` | `boolean` | `false` | Hesap nitelikli satırlar etkinken otomatik Pool/Direct yerel satırlarını da yanlarında gösterir. Yalın satırlar normal Pool yönlendirmesi ve fallback kullanır; nitelikli satırlar yalnızca seçilen hesabı kullanır. |
