...
| `codexAccountPickerShowPoolModels?` | `boolean` | `false` | account-qualified 行が有効なとき、通常の自動 Pool/Direct native 行も同時に表示します。bare 行は通常の Pool routing と fallback を使用し、qualified 行は指定アカウントだけを使用します。 |
