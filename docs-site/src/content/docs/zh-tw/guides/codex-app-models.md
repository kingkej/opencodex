...
列，並預設從 Codex 選擇器中隱藏裸的原生列。設定 `codexAccountPickerShowPoolModels: true` 後，
自動 Pool/Direct 列會和帳號限定列同時顯示。選擇器標籤是使用者自訂的公開名稱，本身沒有帳號角色
...
| Codex 登入（啟用帳號限定列且有合格選擇器） | 每個合格選擇器與受支援的原生模型各有一列 `<selector>/<native-openai-model>`；每列只使用其對應帳號。裸原生列預設隱藏；設定 `codexAccountPickerShowPoolModels: true` 後，會繼續顯示為自動 Pool/Direct 選項。原生後設資料與 context 視窗保持不變。 |
