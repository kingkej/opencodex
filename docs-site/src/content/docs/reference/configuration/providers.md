...
| `requiresCodexTurnMetadataPassthrough?` | `boolean` | Keep Codex's private `internal_chat_message_metadata_passthrough` item metadata on `input[*]` for a noncanonical `openai-responses` sidecar that rebuilds a native Codex turn — the ChatGPT web bridge finds the turn's `<environment_context>` (and its `cwd`) by that turn id. Leave absent for ordinary gateways, which reject the field as an unknown `input[*]` parameter. |
| `supportsCodexResponsesCompaction?` | `boolean` | Explicitly opt a noncanonical `openai-responses` sidecar into Codex's private remote-compaction contract (`compaction_trigger` in, exactly one `compaction` item out). Leave absent for ordinary Responses gateways: OpenCodex will safely drive those as prose summarizers. Enable only when the upstream implements the complete contract; a Responses-shaped API alone is not sufficient. |
