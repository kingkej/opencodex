...
| `codexAccountNamespaces?` | `Record<string, string>` | — | Optional map from an arbitrary public model selector to a stored Codex account target. When account-qualified picker rows are enabled, each selector whose target is present adds separate `<selector>/<native-openai-model>` rows to the Codex picker; each row uses only that account. Bare native rows are hidden by default while selectors are active, but remain routable and listed by raw `/v1/models` unless explicitly disabled. |
...
| `codexAccountPickerShowPoolModels?` | `boolean` | `false` | When account-qualified rows are enabled, keeps bare native Pool/Direct rows visible alongside them. Bare rows use normal Pool/Direct routing and fallback; selector-qualified rows remain exact-account routes that fail closed. Has no catalog effect while account-qualified rows are disabled. |
...
The Codex Auth dashboard controls own maps that have an explicit `codexAccountPickerEnabled` field.
...
selector instead of allocating a new one. Enable `codexAccountPickerShowPoolModels` to present both
automatic Pool choices and explicit account choices in the same picker.
