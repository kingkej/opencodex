...
`<selector>/<native-openai-model>` и по умолчанию скрывает bare native-строки из picker'а.
`codexAccountPickerShowPoolModels: true` оставляет автоматические строки Pool/Direct видимыми рядом
с account-qualified строками. Имена селекторов —
