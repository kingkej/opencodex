...
| `codexAccountPickerShowPoolModels?` | `boolean` | `false` | При включённых account-qualified строках оставляет рядом обычные автоматические native-строки Pool/Direct. Bare-строки используют штатную маршрутизацию и fallback Pool, а qualified-строки — только указанный аккаунт. |
