...
| `codexAccountPickerShowPoolModels?` | `boolean` | `false` | Lorsque les lignes qualifiées par compte sont activées, conserve les lignes natives Pool/Direct automatiques à côté d'elles. Les lignes nues utilisent le routage et le fallback normaux du Pool ; les lignes qualifiées restent liées à un compte exact. |
