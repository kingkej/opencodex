...
| --- | --- |
| Config/settings | Read safe config/settings views; mutate supported settings only. Full `PUT /api/config` is disabled so masked secrets are not round-tripped. `PUT /api/settings` accepts `codexAutoStart`, `streamMode`, integer `appOwnedMemoryBudgetMb` (64..4096), and/or strict booleans `codexAccountPickerEnabled` and `codexAccountPickerShowPoolModels` (each optional, at least one required). Picker enable initializes an empty UI-managed selector map; the Pool-model flag is a stored presentation preference that remains inert while the picker is disabled and otherwise keeps bare automatic rows beside exact-account rows. Changes persist before one bounded catalog convergence and report only `catalogRefreshPending`; allocation/save failure restores every touched live field and skips convergence. Budget changes synchronously enforce the process-wide evictable retained-state cap; this is separate from RSS/native memory. `streamMode` persists the #314 stream-shape selection in config.json (Windows services need persisted input; macOS eager relay is explicit-only). |
