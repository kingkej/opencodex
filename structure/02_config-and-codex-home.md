...
| Catalog | `disabledModels`, `customModels`, `modelCacheTtlMs`, `providerContextCaps`, `contextCapValue`, `codexAccountNamespaces`, `codexAccountPickerEnabled`, `codexAccountPickerShowPoolModels` | Catalog state is derived; config only records intent. The picker flags control account-row and automatic Pool-row visibility, while selector mappings remain the durable exact-routing contract. |
